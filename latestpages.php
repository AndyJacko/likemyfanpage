...
if (!isset($_GET["page"])) {
  ...
  $query_RSLatestPages = "SELECT * FROM tbl_pages WHERE page_active=1 ORDER BY tbl_id DESC LIMIT 10";
  $RSLatestPages = mysql_query($query_RSLatestPages, $Wisdom_Mcr) or die(mysql_error());
  $row_RSLatestPages = mysql_fetch_assoc($RSLatestPages);
  $totalRows_RSLatestPages = mysql_num_rows($RSLatestPages);
} else {
  mysql_select_db($database_Wisdom_Mcr, $Wisdom_Mcr);
  $query_RSLatestPages = "SELECT * FROM tbl_pages WHERE tbl_id=".$_GET["page"];
  $RSLatestPages = mysql_query($query_RSLatestPages, $Wisdom_Mcr) or die(mysql_error());
  $row_RSLatestPages = mysql_fetch_assoc($RSLatestPages);
  $totalRows_RSLatestPages = mysql_num_rows($RSLatestPages);
}
...
<meta name="description" content="View the latest additions to the LikeMyFanPage family. These are the latest Facebook Pages added by our fans.">
<meta name="keywords" content="">
<!-- InstanceEndEditable -->
<!-- InstanceBeginEditable name="doctitle" -->
<title>View the latest additions to the LikeMyFanPage family.</title>
<!-- InstanceEndEditable -->
    ...
    <div id="contentCol" class="clearfix">
      <div id="headerArea">
		...
		<!-- InstanceBeginEditable name="pagebody" -->
          <p><strong><?php if (!isset($_GET["page"])) { echo "View the 10 latest additions to the LikeMyFanPage family."; } else { echo "Welcome the latest addition to the LikeMyFanPage family."; } ?></strong></p>
		  <?php do { ?>
			<?php if (!isset($_GET["page"])) { ?>
              <div class="frontPageBoxes"><div id="fb-root"></div><script src="http://connect.facebook.net/en_US/all.js#xfbml=1"></script><fb:like-box href="<?php echo $row_RSLatestPages['page_url'] ?>" width="250" show_faces="true" border_color="" stream="false" header="false"></fb:like-box><div class="reportPage"><a onClick="confirmation('/reportbug.php?id=<?php echo $row_RSLatestPages['tbl_id']; ?>','ReportDeadLink','width=300,height=100')"><span class="img rbugicon"></span></a></div></div>
			<?php } else { ?> 
              <div class="frontPageBoxes"><div id="fb-root"></div><script src="http://connect.facebook.net/en_US/all.js#xfbml=1"></script><fb:like-box href="<?php echo $row_RSLatestPages['page_url'] ?>" width="500" show_faces="true" border_color="" stream="false" header="false"></fb:like-box><div class="reportPage"><a onClick="confirmation('/reportbug.php?id=<?php echo $row_RSLatestPages['tbl_id']; ?>','ReportDeadLink','width=300,height=100')"><span class="img rbugicon"></span></a></div></div>
            <?php } ?>
          <?php } while ($row_RSLatestPages = mysql_fetch_assoc($RSLatestPages)); ?>
		<!-- InstanceEndEditable -->
      </div>
      <div id="rightAds">
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($RSLatestPages);
?>